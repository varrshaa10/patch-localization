...
"""
import argparse, os, csv, random, json
import numpy as np
...
def generate_pair(architecture, pair_id, out_dir, add_marker=True, mode="L", jitter=False):
    ...
    pair_dir = os.path.join(out_dir, f"pair_{pair_id}")
    os.makedirs(pair_dir, exist_ok=True)
    mode_suffix = "_rgb" if mode == "RGB" else ""
    ref_path_abs = os.path.join(pair_dir, f"reference{mode_suffix}.png")
    search_path_abs = os.path.join(pair_dir, f"search{mode_suffix}.png")
    ref_path_csv = f"../tests/synthetic_data/pair_{pair_id}/reference{mode_suffix}.png"
    search_path_csv = f"../tests/synthetic_data/pair_{pair_id}/search{mode_suffix}.png"
...
def main():
    ...
    combined_csv_path = os.path.join(args.output_dir, "ground_truth_combined.csv")
    with open(combined_csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "pair_id", "architecture", "ref_path", "search_path",
            "gt_x", "gt_y", "angle", "scale", "add_marker", "mode"
        ])
        writer.writeheader()
        for row in rows:
            combined_row = {
                ...
                "ref_path": f"../tests/synthetic_data/pair_{row['pair_id']}/reference.png",
                "search_path": f"../tests/synthetic_data/pair_{row['pair_id']}/search.png",
                "gt_x": row["gt_x"],
                "gt_y": row["gt_y"],
                "angle": row["angle"],
                "scale": row["scale"],
                "add_marker": row["add_marker"],
                "mode": row["mode"],
            }
            writer.writerow(combined_row)

    for row in rows:
        pair_dir = os.path.join(args.output_dir, f"pair_{row['pair_id']}")
        json_path = os.path.join(pair_dir, "ground_truth.json")
    ...
    print(f"\nSaved {len(rows)} pairs + ground_truth_combined.csv to {args.output_dir} (mode={mode}{jitter_info})")
