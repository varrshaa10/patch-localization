...
python score_synthetic_phase2.py
```

## Evaluation And Submission

There are three distinct datasets and purposes. Do not combine their scores.

### 1. Our 100-pair generated dataset

This is the primary local algorithm evaluation. It is generated by us and is
used to test nominal cases, degraded cases, absent-reference rejection, and
runtime:
...
`score_synthetic_phase2.py` reads the generated ground truth, predictions, and
timings by default. It reports 52 TP, 25 FP, 8 FN, and 15 TN, with rejection
F1 `0.759124` and 26/60 present pairs localized within 5 px. This is a local
development score, not an organizer score. Its category results were nominal
present: 25/30 found, mean credit `0.553`, 17 within 5 px; degraded present:
27/30 found, mean credit `0.280`, 9 within 5 px; and absent: 25/40 rejected.
Median runtime was `1.448588s`, with a maximum of `1.943961s`. Use
`--ground_truth`, `--predictions`, and `--timings` to score a different
generated dataset.

### 2. The checked-in 20-pair fixture
...
### 3. The actual organizer dataset
...
The 80-pair dataset and the 20-pair RGB Set-D-style dataset are older or
specialized local experiments. Their results should not be compared directly
with the 100-pair result or reported as organizer results.
