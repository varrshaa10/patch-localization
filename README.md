...
## ML Experiments (Not Used in Production)

Two experimental DL reranker attempts were tested and both were found not to outperform the classical NCC pipeline: a binary classification reranker and a regression reframe. The classification experiment reached a patch-level ranking score of about 0.916 but regressed the no-marker end-to-end pixel error from about 40 px to about 195 px, and the regression proof run showed no meaningful learning signal. See [algorithm/dl/RESULTS.md](algorithm/dl/RESULTS.md) for the detailed honest record.
...
- `scikit-learn` — Optional calibration experiment only; not required for the core NCC pipeline
...
**Output:**
- Each pair is created in its own folder: `<output_dir>/pair_N/reference.png`, `<output_dir>/pair_N/search.png`, and `<output_dir>/pair_N/ground_truth.csv`
- The full combined CSV is kept at `<output_dir>/ground_truth_combined.csv` with columns:
...
This generates per-pair RGB image folders and a combined CSV in:
...
```bash
cd algorithm/core
python infer.py --reference ../tests/synthetic_data_rgb/pair_2/reference_rgb.png --search ../tests/synthetic_data_rgb/pair_2/search_rgb.png
```
...
**Example:**
```bash
python infer.py --reference ../tests/synthetic_data/pair_0/reference.png --search ../tests/synthetic_data/pair_0/search.png
```
...
│   │       │   └── ground_truth.csv
│   │       ├── ...
│   │       └── ground_truth_combined.csv  # Full 60-row CSV for batch eval
│   │
│   ├── docs/                          # Documentation
...
[Download Project ZIP](https://drive.google.com/file/d/1A-lyJkwbOrboGI3CJizZCzMYBTh03gav/view?usp=sharing)

Complete project package containing the source code, dataset generator, localization algorithm, evaluation files, and supporting materials.

### Demonstration Video

[View Project Demonstration Video](https://drive.google.com/file/d/1ZqhN1GQDWdBO-5VYBj3xeM470oZAcds9/view?usp=sharing)

Video demonstration showing the localization pipeline running on a sample image pair and displaying the predicted result.
